...
// Save all steps entries (atomic write; callers hold the per-file lock)
async function saveStepsEntries(userId: string, entries: StepsEntry[]): Promise<void> {
  await writeJsonAtomic(getStepsPath(userId), entries);
}
...
export async function createStepsEntry(userId: string, steps: number, date?: string, timestamp?: string, notes?: string): Promise<StepsEntry> {
  return withFileLock(getStepsPath(userId), async () => {
